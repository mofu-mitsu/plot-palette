...
export async function ensureTablesInitialized() {
  const db = getDb();
  if (!db || !sqlClientInstance) return;
  if (initialized) return;
  if (initPromise) {
    try {
      await initPromise;
    } catch (e) {
      // Ignore inner throw, let retry mechanism handle
    }
    return;
  }
  try {
    await initializeTables(sqlClientInstance);
  } catch (err) {
    console.error("[DB Debug] ensureTablesInitialized retry failed:", err);
  }
}
